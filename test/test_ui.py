...
def test_search_by_year(search_page):
    ...
    with allure.step("В методе указываем год"):
        search_page.search_by_year(2022)
    ...
    with allure.step("Метод проверяет, что контент соответствует году"):
        ...
        assert year_element.text == "2022", f"Expected year to be 2022, but got {year_element.text}"


@allure.title("Поиск сериала на кириллице (Позитивный) ")
@allure.description("Тест на поиск сериала по названию.")
@allure.id(2)
@allure.severity("Критический")
def test_search_by_title(search_page):
    
    with allure.step("В методе указываем название сериала'"):
        search_page.search_by_title("Сёгун")
    
    with allure.step("Метод проверяет, что контент соответствует искомому сериалу."):
        is_content_found = search_page.wait_for_element_with_text(By.CLASS_NAME, "gray", "Shôgun")
        assert is_content_found, "Expected to find content 'Shôgun'"

@allure.title("Поиск сериала на кириллице (Негативный) ")
@allure.description("Тест на поиск несуществующего сериала по названию.")
@allure.id(3)
@allure.severity("Критический")
def test_search_title_negativ(search_page):
    
    with allure.step("В методе указываем название сериала'"):
        search_page.search_by_title("I'm breaking into the world of IT")
    
    with allure.step("Метод проверяет, что контент соответствует искомому сериалу."):
        is_content_found = search_page.wait_for_element_with_text(By.CLASS_NAME, "textorangebig", "К сожалению, по вашему запросу ничего не найдено...")
...
def test_search_by_genre(search_page):
    ...
    with allure.step("Метод проверяет, что найденный контент относится к искомому жанру"):
        is_genre_found = search_page.wait_for_element_with_text(By.XPATH, "//span[@class='gray' and contains(., 'история')]", "история")
        assert is_genre_found, "Expected to find content with genre 'история'"


@allure.title("Поиск фильма и персоны")
@allure.description("Выполнение теста на поиск контента по названию фильма и актеру")
@allure.id(5)
@allure.severity("Критический")
def test_search_by_actor(search_page):
    ...
    with allure.step("Метод проверяет, что найденный контент соответствует указанному."):
        is_content_found = search_page.wait_for_element_with_text(By.CLASS_NAME, "gray", "Constantine")
